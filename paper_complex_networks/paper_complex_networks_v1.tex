...
Let $f_{st}=n_{st}/n_{\bullet\bullet}$ be the proportion of $st$-trips (empirical distribution) and let $g_{st}$ be some prior guess on its shape (theoretical distribution). Also suppose a prior guess of row and margin constraints for $f_{st}$, respectively $c_s$ and $d_t$, which has to verify 
\begin{equation}
	\frac{c_i}{c_j} = \frac{a_i}{a_j} \qquad \frac{d_i}{d_j} = \frac{b_i}{b_j}  \qquad \forall i, j \text{ without junction.} \\
\end{equation}
Setting these gives us
\begin{equation}
	n_{\bullet \bullet} = \frac{c_i}{a_i} = \frac{d_i}{b_i} \qquad \forall i \text{ without junction.}  
\end{equation}
