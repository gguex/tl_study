...
\begin{equation}
\text{vec}(\mathbf{X}) = \mathbf{S}^\top \text{vec}(\mathbf{N}),
\end{equation}
where $\text{vec}(.)$ denotes the vectorization function of a matrix, obtained by stacking matrix columns on top of one another.
