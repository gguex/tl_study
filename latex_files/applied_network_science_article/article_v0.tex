...
\label{algo}
\begin{algorithm}[H]
	\caption{Compute the transportation flow matrix $\mathbf{N} = (n_{st})$ knowing the edge-trip incidence matrix $\bm{\chi} = (\chi_{ij}^{st})$, the embarking flow $\mathbf{a}$, the disambarking flow $\mathbf{b}$, the index of an isolated source node $\tilde{s}$, and the minimum proportion of passengers entering/leaving the network hyperparameter $\theta$.}
	\label{algo1}
	\begin{algorithmic}[1]
		...
		\State $\alpha_s \leftarrow a_s / a_\bullet ,\; \beta_t \leftarrow b_t / b_\bullet, \; \forall s,t$ \Comment{{\footnotesize Initialize the margin distributions.}}
		...
		\While{$\mathbf{N} = (n_{st})$ has not converge} \Comment{{\footnotesize Main loop.}}
			\State $\psi_t \leftarrow 1, \; \forall t$ \Comment{{\footnotesize Initialize the lagragian multiplier for destinations.}}
